...
def extract_frame(frames_path, outname, outdir_img, class_mapping, resize_ratio=1.0):
    ...
    for fidx, data in enumerate(dataset):
        ...
        try:
            ...
            print("Error in parsing range image and camera projection - {}".format(fidx))
            continue
...
def download(args, seg_id, labels_dir, images_dir, clip_id=0, wod_ver='1_2_0'):
    ...
    segid_dir = os.path.join(args.out_dir, args.split + '_%04d'%seg_id)
    os.makedirs(segid_dir, exist_ok=True)
